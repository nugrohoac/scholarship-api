...
func (m majorSuite) TestMajorRepoFetch() {
	...
	// without filter
	response, cursor, err := majorRepo.Fetch(context.Background(), sa.MajorFilter{})
	require.NoError(t, err)
	require.Equal(t, "MjAyMi0wMi0xOVQxNToyODo0NS40NTla", cursor)
	require.Len(t, response, 3)

	// limit 1
	response, cursor, err = majorRepo.Fetch(context.Background(), sa.MajorFilter{Limit: 1})
	require.NoError(t, err)
	require.Equal(t, "MjAyMi0wMi0xOVQxNToyODo1NS40NTla", cursor)
	require.Len(t, response, 1)

	// cursor
	response, cursor, err = majorRepo.Fetch(context.Background(), sa.MajorFilter{Limit: 2, Cursor: cursor})
	require.NoError(t, err)
	require.Equal(t, "MjAyMi0wMi0xOVQxNToyODo0NS40NTla", cursor)
	require.Len(t, response, 2)

	// name
	response, cursor, err = majorRepo.Fetch(context.Background(), sa.MajorFilter{Name: "theMAti"})
	require.NoError(t, err)
	fmt.Println(cursor)
	require.Equal(t, "MjAyMi0wMi0xOVQxNToyODo0NS40NTla", cursor)
	require.Len(t, response, 1)
