...
func (c countrySuite) TestCountryRepoFetch() {
	...
	countriesResp, cursor, err := countryRepo.Fetch(context.Background(), entity.CountryFilter{})
	require.NoError(c.T(), err)
	require.Equal(c.T(), "MjAyMi0wMS0xMVQwNTozNDowMy4xOTFa", cursor)
	require.Equal(c.T(), 3, len(countriesResp))

	// limit
	countriesResp, cursor, err = countryRepo.Fetch(context.Background(), entity.CountryFilter{Limit: 1})
	require.NoError(c.T(), err)
