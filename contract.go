...
type ScholarshipRepository interface {
	...
	Apply(ctx context.Context, userID, scholarshipID int64, applicant int, essay string, recommendationLetter Image) error
}
...
type ScholarshipService interface {
	...
	GetByID(ctx context.Context, ID int64) (Scholarship, error)
	Apply(ctx context.Context, userID, scholarshipID int64, essay string, recommendationLetter Image) (string, error)
}
